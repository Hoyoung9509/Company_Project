...
## `.bat` 스크립트에 한글을 쓰면 안 되는 이유

- `cmd.exe`는 `.bat` 파일을 UTF-8이 아니라 **시스템 활성 코드페이지(한글 Windows는 CP949)** 기준으로 읽는다.
- 파일이 UTF-8(BOM 없음)로 저장돼 있으면 한글 멀티바이트 시퀀스가 CP949로 잘못 해석되면서 문자 경계가 깨지고, 그 깨진 바이트가 우연히 배치 특수문자(`"`, `%`, `&` 등)처럼 보이는 경우 **파싱 자체가 어긋나서 스크립트 뒷부분까지 전부 이상하게 실행**된다 (`'?'은(는) 내부 또는 외부 명령...` 같은 알 수 없는 명령 에러가 여러 줄 연속으로 뜨는 게 전형적인 증상).
- `chcp 65001`로 우회할 수도 있지만 리다이렉션/일부 명령과 상호작용이 불안정하다.
- **가장 확실한 해결책: `.bat` 파일에는 한글을 쓰지 않고 영어로만 작성한다.** `start.bat`/`stop.bat`/`logs.bat`/`cloudflare-*.bat` 전부 이 이유로 영문 메시지만 사용함.
